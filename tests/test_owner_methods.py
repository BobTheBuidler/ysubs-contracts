...
def test_create_plan(subscriber_contract, accounts):
    ...
    for i in range(1, MAX_POOL_CT):
        ...
        with brownie.reverts("You are not the owner."):
            subscriber_contract.create_plan(amount, RATE_LIMIT, TIME_INTERVAL, _from(bad_guy))
        subscriber_contract.create_plan(amount, RATE_LIMIT, TIME_INTERVAL, _from(owner))
